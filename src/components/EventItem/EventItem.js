  ...
  renderFavButton = () => {
    if (!TokenService.hasAuthToken()) {
      ...
      return (
        <div className='EventItem__goLogin'>  
          <Link className='button_login' to='/signIn'>Login to Save Favorites</Link>          
        </div>
      );
    }
    const inFavorites = this.context.savedEvents.find(savedEvent => savedEvent.fetch_id === this.props.fetch_id)
    if (inFavorites) {
      ...
      return (
        <div className='EventItem__unfavorite'>  
          <button className='button_unfavorite'  onClick={() => this.context.removeEvent(inFavorites.id)} >Remove From Favorites</button>          
        </div>
      );
    }
    ...
    return (
      <div className='EventItem__favorite'>  
        <button className='button'  onClick={() => this.context.saveEvent(this.props)} ></button>          
      </div>
    );
  }
